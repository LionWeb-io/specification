...
For all features (i.e. `properties`, `children`, and `references`) defined for a **node**'s classifier:{fn-org33}

* During serialization, we SHOULD include every feature, even if unset.
In the latter case:
** the *property* MUST have `value` = _null_;
** the *child* MUST have `children` = _empty array_;
...
* We CAN accept undefined features during deserialization if we can deal with them in a meaningful manner.

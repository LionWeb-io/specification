...
* [[Node.classifier]] _name_ `classifier`{fn-org37-name}{fn-org184} with _object_ _value_, adhering to <<meta-pointer>>.
  The *meta-pointer*'s ``key``'s _value_ refers to the <<{m3}.adoc#IKeyed.key, *key*>> of the <<{m3}.adoc#Annotation, *Annotation*>> or non-abstract <<{m3}.adoc#Concept, *Concept*>> this *node* is an instance of.
...
+
`resolveInfo` and `reference` MUST NOT both be `null`.
The `reference` value has preference over the `resolveInfo` value in the following sense:
if the `reference` value resolves to a node _n~1~_ and the `resolveInfo` value resolves to a different node _n~2~_, then the reference (as a whole) MUST resolve to _n~1~_.{fn-org96}{fn-org396}
...
* <<reference, references>> on `resolveInfo` vs. `reference`.{fn-org396}
* <<parent, parent>> `null` conditions{fn-org228}

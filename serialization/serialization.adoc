...
The first member SHOULD be _key_ `serializationFormatVersion` with a _string_ _value_.{fn-org58}
...
The second member SHOULD be _key_ `languages` with an _array_ _value_.{fn-org76}{fn-org78}
...
The third member SHOULD be _key_ `nodes` with an _array_ _value_.{fn-org33}
...
+
NOTE: *parent* is the inverse relation of either one *child* or one *annotation*.
+
NOTE: The referred *node* CAN be contained in the processed document, but also CAN be outside the processed document (i.e. not contained in the processed document).

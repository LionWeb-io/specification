...
An M1 instance of StructuredDatatype MUST have non-null values for all its fields with `type` of <<PrimitiveType>> or <<Enumeration>>.
fields with `type` of StructuredDatatype can be `null`.

A StructuredDatatype can (directly or indirectly) contain a field with itself as `type`.

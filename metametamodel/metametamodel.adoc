...
The classes are <<Language>>, <<Concept>>, <<Annotation>>, <<ConceptInterface>>, <<PrimitiveType>>, <<Enumeration>>, <<EnumerationLiteral>>, <<Containment>>, <<Reference>>, and <<Property>>.
...
In EMF, an `Adapter` can be used for similar purposes.

.Characteristics
An Annotation has a <<INamed.name, `name`>> and an <<IKeyed.key, `key`>>.
It can be concrete (i.e., instantiable) or abstract, marked by boolean <<Concept.abstract, `abstract`>> property.

[[Annotation.multiple, Annotation.multiple]]
An Annotation can specify whether an annotated node can have more than one annotation via the boolean `multiple` property.

An Annotation is a <<Concept>>, and indirectly a <<LanguageEntity>> (as it is a top level element in a <<Language>>), an <<INamed>> (as it has a name), a <<IKeyed>> (as it has a key).
...
As Annotations are Concepts, they can specify which Concept or Annotation it <<Concept.extends, `extends`>> and which interfaces it <<Concept.implements, `implements`>>.
An Annotation can have any number of <<Classifier.features, `features`>>, given it is a <<Classifier>>.

.Constraints
Although Annotation inherits <<Concept.partition, `partition`>> property, this flag MUST always be `false`.
Annotations as <<partition>> makes no sense, as they annotate another node (and never stand for themselves as root).
...
class AliceDocu2 <<a, orange>><<DocumentationAnnotation>> ##red {
  role = "invalid because\nDocumentationAnnotation.multiple=false"
}
Alice <-[#red]- AliceDocu2
...
We SHOULD avoid mixing Annotations and Concepts in an inheritance hierarchy, and containing Annotations as regular children.
It is technically possible, but confusing.

.Undesired (but possible) example of mixing Annotations and Concepts inheritance, and annotations as children
[plantuml, mixed-annotation-concept-inheritance, svg]
----
hide empty members

class NatureTree <<C, lightgreen>><<Concept>> {
  preferredSeason: String
}

class StrangeWood <<C, lightgreen>><<Concept>>

StrangeWood "1" *- "*" StrangeTree: trees



annotation StrangeTree <<A, orange>><<Annotation>> extends NatureTree {
  multiple = true
  --
  color: String
}

class Toy <<C, lightgreen>><<Concept>> {
  minAge: Integer
}

StrangeTree -> Toy: annotates


class SmallTree <<C, lightgreen>><<Concept>> extends StrangeTree

annotation VerySmallTree <<A, orange>><<Annotation>> extends SmallTree {
  multiple = false
}

class Pet <<C, lightgreen>><<Concept>>

VerySmallTree -> Pet: annotates
----
...
.Characteristics
A ConceptInterface has a <<INamed.name, `name`>> and an <<IKeyed.key, `key`>>.

A ConceptInterface is an <<Classifier>>  (as it has features).
It is indirectly a <<LanguageEntity>> (as it is a top level element in a <<Language>>), an <<INamed>> (as it has a name), a <<IKeyed>> (as it has a key).

[[ConceptInterface.extends, ConceptInterface.extends]]
Each ConceptInterface `extends` zero or more ConceptInterfaces.

A ConceptInterface can have any number of <<Classifier.features, `features`>>, given it is a <<Classifier>>.
...
The abstract classes are <<LanguageEntity>>, <<Classifier>>, <<DataType>>, <<Feature>>, and <<Link>>.
...
|<<Annotation.multiple>> |Property |Annotation-multiple
...
|<<IKeyed>> |ConceptInterface |IKeyed
...
|<<Annotation>>
|link:https://download.eclipse.org/modeling/emf/emf/javadoc/2.11/org/eclipse/emf/common/notify/Adapter.html[Adapter]

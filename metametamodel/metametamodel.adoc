...
[[StructuredDatatype]]
==== StructuredDatatype
Represents a collection of named instances of <<PrimitiveType, Primitive Types>>.
They are meant to support a small composite of values that semantically form a unit.
StructuredDatatypes have no identity, are always copied by value, and SHOULD be immutable.{fn-org265}

#TODO: use proper name for concept#
...
.Constraints
Instances of StructuredDatatype SHOULD be immutable.

An instance of StructuredDataype MUST have non-null values for all its fields with a `type` of <<PrimitiveType>> or <<Enumeration>>.
fields with a `type` of StructuredDatatype MAY be `null`.

A StructuredDatatype MAY (directly or indirectly) contain a field with itself as `type`.
...
A StructuredDatatype had the fields "red", "green", and "blue".
...
A Field is a <<IKeyed>> (as it has a key) and indirectly an <<INamed>> (as it has a name).

.Constraints
Each Field must have a unique `name` within the containing StructuredDatatype.
...
|<<StructuredDatatype>> |Concept |StructuredDatatype
|<<StructuredDatatype.fields>> |Containment |StructuredDatatype-fields
|===

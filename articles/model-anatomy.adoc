...
Any time we refer from an instance model (M1) to its metamodel (M2 a.k.a. _Language_), we use meta-pointers.
...
In any case, we store all of them as string in low-level serialization format (i.e. JSON).
...
* In the <<space-project, space project example above>>, assume some app that displays the list of external parties for the project as `ESA: 23, JAXA: 42`.
...
Containments establish a tree shape for the model.
With references, we regard the model as a graph, as a node can be target of several references.
...
For the domain of `House`, we don't want to bother with the details how a parser constructed that node.
...
But we've just added the very domain-specific `windows` as annotations to a `House` -- doesn't that smell the same?
Yes.
Ideally, we would work with ``House``'s designer to add windows, or create a specialized concept `WindowedHouse extends House`.
We might resort to windows as annotations as a trade-off if we interchanged house models with external parties.

...
Assuming an implementation in an object-oriented language like Java, something went wrong if we had e.g. two different Java objects of class `Node`, but with equal `nodeId` field.
...
This library contains our pre-defined <<primitive-types, primitive types>>, `Node` as the superconcept of all concepts, and interface `INamed`.
...
== Languages
Programming languages like java natively know things like classes, records, and enums.
...
A generalization, (a.k.a. supertype, superclass, extended class, implemented interface) is the opposite of _specialization_ (a.k.a. subtype, subclass, interface implementations).
...
The linked sections describe the details of all features.

...
#TODO Change `Language.name` to `INamed.name` and mention implementation relation.#
...
// [[UsedLanguage]]
// For each dependency, a Language contains one `UsedLanguage` instance.
// A UsedLanguage has a [[UsedLanguage.key, UsedLanguage.key]]`key` and a [[UsedLanguage.version, UsedLanguage.version]]`version` property, pointing to the other Language this Language depends on.{fn-org129}
...
#TODO Change `NamespacedEntity.name` to `INamed.name` and mention implementation relation.#
...
The language hosting built-in elements has key `LIonCore-builtins`.
...
+
This interface CAN be used to resolve references by name.
...
It is defined by means of itself, as outlined by link:https://en.wikipedia.org/wiki/Meta-Object_Facility[Meta-Object Facility].

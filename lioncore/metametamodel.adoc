...
[[Language]]
==== Language
A Language will provide the Concepts necessary to describe ideas in a particular domain together with supporting elements necessary for the definition of those Concepts.
...
.Characteristics
A Language has a [[Language.name, Language.name]]`name`, a [[Language.key, Language.key]]`key`, and a [[Language.version, Language.version]]`version`, similar to MPS Languages.
...
A NamespaceEntity's `key` must be unique within the Language.
``key``s must a valid <<identifiers>>.

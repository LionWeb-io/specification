...
.Characteristics
A Metamodel has a `qualifiedName` and an `id`, similarly to MPS Languages.
...
.Characteristics
A Concept has a <<simpleName, `simpleName`>> and an <<id, `id`>>.
A Concept has a reference <<container, `container`>> to the containing <<Metamodel>>.
...
A Concept can have any number of <<features, `features`>>, given it is a <<FeaturesContainer>>.
...
.Characteristics
A ConceptInterface has a <<simpleName, `simpleName`>> and an <<id, `id`>>.
A ConceptInterface has a reference <<container, `container`>> to the containing <<Metamodel>>.
...
A ConceptInterface can have any number of <<features, `features`>>, given it is a <<FeaturesContainer>>.
...
.Characteristics
A PrimitiveType has a <<simpleName, `simpleName`>> and an <<id, `id`>>.
A PrimitiveType has a reference <<container, `container`>> to the containing <<Metamodel>>.

A PrimitiveType is a <<DataType>> (as it can be used as <<propertyType, type>> of a <<Property>>).
...
.Characteristics
An Enumeration has a <<simpleName, `simpleName`>> and an <<id, `id`>>.
An Enumeration has a reference <<container, `container`>> to the containing <<Metamodel>>.

[[literals]]
An Enumeration contains <<EnumerationLiteral, EnumerationLiterals>> in its `literals` containment.

An Enumeration is a <<DataType>> (as it can be used as <<propertyType, type>> of a <<Property>>) and a <<NamespaceProvider>> (as it act as the namespace for its literals).
It is indirectly a <<MetamodelElement>> (as it is a top level element in a <<Metamodel>>) and a <<NamespacedEntity>> (as it has an identity and it is contained in the namespace of the Metamodel).
...
.Characteristics
An EnumerationLiteral has a <<simpleName, `simpleName`>> and an <<id, `id`>>.
An Enumeration has a reference <<container, `container`>> to the containing <<Enumeration>>.
...
.Characteristics
A Containment has a <<simpleName, `simpleName`>> and an <<id, `id`>>.
A Containment has a reference <<container, `container`>> to the containing <<FeaturesContainer>>.
It can be marked as <<optional, `optional`>> and <<multiple, `multiple`>>.

A Containment refers its <<linkType, `type`>>, which is a <<FeaturesContainer>>.
...
.Characteristics
A Reference has a <<simpleName, `simpleName`>> and an <<id, `id`>>.
A Reference has a reference <<container, `container`>> to the containing <<FeaturesContainer>>.
It can be marked as <<optional, `optional`>> and <<multiple, `multiple`>>.
...
.Characteristics
A Property has a <<simpleName, `simpleName`>> and an <<id, `id`>>.
A Property has a reference <<container, `container`>> to the containing <<FeaturesContainer>>.
It can be marked as <<optional, `optional`>>.
...
[[container]]
A NamespacedEntity has a reference `container` to the containing <<NamespaceProvider>>.
...
.Characteristics
A MetamodelElement has a <<simpleName, `simpleName`>> and an <<id, `id`>>.
A MetamodelElement has a reference <<container, `container`>> to the containing <<Metamodel>>.
...
.EMF & MPS equivalent
FeaturesContainer is similar to `EClass` in Ecore (which is used both for classes and interfaces) and to `AbstractConcept` in MPS.

.Characteristics
A FeaturesContainer has a <<simpleName, `simpleName`>> and an <<id, `id`>>.
A FeaturesContainer has a reference <<container, `container`>> to the containing <<Metamodel>>.
...
.Characteristics
A DataType has a <<simpleName, `simpleName`>> and an <<id, `id`>>.
A DataType has a reference <<container, `container`>> to the containing <<Metamodel>>.
...
.Characteristics
A Feature has a <<simpleName, `simpleName`>> and an <<id, `id`>>.
A Feature has a reference <<container, `container`>> to the containing <<FeaturesContainer>>.
...
.Characteristics
A Link has a <<simpleName, `simpleName`>> and an <<id, `id`>>.
A Link has a reference <<container, `container`>> to the containing <<FeaturesContainer>>.
It can be marked as <<optional, `optional`>>.
...
.Characteristics
LionCore represents multiplicity as the two booleans <<optional, `optional`>> (whether there MUST be at least one target) and <<multiple, `multiple`>> (whether there CAN be more than one target).

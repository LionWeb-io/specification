...
A Language will not have a version number, differently from MPS Languages.
...
`version` must be >= 1, and never decrease.
It should increment linearly by one.

Language elements contained in a Language are allowed to refer exclusively to Language elements contained in the same Language or in Language explicitly declared as dependencies by the current Language.

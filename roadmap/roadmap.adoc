...
The first such protocol will be the https://github.com/LionWeb-org/organization/blob/d8a54813c6bd224ef84e53a26ac7d60a45a866b5/bulk/repo-access-api.adoc[Bulk Repository access API].
...
Open questions for this protocol include

* Which messages do we support?
For example, is _move a child from position 2 to position 5 in the same list_ the same as _move a child from parent A to parent B_ the same message kind?

* Is the protocol the same for client -> repository updates vs. repository -> client updates?
That's especially relevant for (web-based) editors.

* How strong do we couple the protocol to an implementation?
Both https://en.wikipedia.org/wiki/Operational_transformation[operational transformation] and https://en.wikipedia.org/wiki/Conflict-free_replicated_data_type[CRDTs] can be used to implement consistent clients.
For both of them, several implementations are available.
Additionally, we might support simpler clients that assume no conflicts or missing updates at all.
...
Similar to validation, the baseic structure of scoping is very similar in most systems; this _base language_ would condense the common concepts.
...
===== Commenting / Review notes
For any kind of collaboration, it's very helpful for reviewers to attach their remarks directly to their context -- think of the blue speech bubbles in the right margin of a Word document.
...
A repository might support version control and/or live collaboration of users (both features are technically related).
...
==== Shared annotations in standard library
We might add some <<{m3}.adoc#Annotation, Annotations>> to the <<{m3}.adoc#stdlib, built-in standard library>>, e.g. range constraints on integers, implementation per platform, `@Deprecated`, etc.{fn-org12}

==== Support ordered / unordered containments / references
For some use cases (e.g. merging), it might be useful to know that the order of featues does not matter.{fn-org4}
...
=== 2023.1
Stable <<{serialization}.adoc, serialization format>> and <<{m3}.adoc, meta-metamodel>> describing <<{m3}.adoc#Language, Languages>>, <<{m3}.adoc#Concept, Concepts>>, <<{m3}.adoc#ConceptInterface, ConceptInterfaces>>, <<{m3}.adoc#Annotation, Annotations>>, <<{m3}.adoc#Enumeration, Enumerations>>, <<{m3}.adoc#PrimitiveType, PrimitiveTypes>>, <<{m3}.adoc#Property, Properties>>, <<<<{m3}.adoc#Containment, Containments>>, and <<{m3}.adoc#Reference, References>>.
The standard library contains <<{m3}.adoc#Node, Node>> and <<{m3}.adoc#INamed, INamed>>.
...
* Set up LionWeb-org GitHub organization
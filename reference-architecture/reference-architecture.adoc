...
A *model* is a graph structure with nodes and edges and one primary containment hierarchy [.refletter]#B#.
...
A *repository* [.refletter]#A# stores models and provides clients [.refletter]#C#, [.refletter]#D#, [.refletter]#E# ,[.refletter]#F#, [.refletter]#G# access to the nodes in a model.
...
*Original models* are models that cannot be (re-)computed from other models.
...
A *client* is any program that works with models and nodes in the repository.
...
An *editor* CRUDs original models based on its direct interactions with users.
A *processor* is a client that CRUDs models without direct user interaction.
...
*Bulk* communication means that a client requests a set of nodes from the repository, processes those in isolation (e.g., import or otherwise create new nodes, modifies existing nodes, or generates output) and then writes the changed set of nodes.
*Delta* communication means that, after receiving an initial snapshot similar to the bulk case, the client continues to receive changes (aka deltas) from the repo; it also writes back changes continuously.

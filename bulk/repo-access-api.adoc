...
== Response Format
The repository replies each command with a JSON object.{fn-org236}
It contains two required properties `success` and `messages`.
Depending on the command, it might contain a `chunk` property.

.Example (with empty chunk)
[source,json]
----
include::exampleBulkResponse.json[]
----

`success` is a boolean flag to state whether the command executed successfully.

`chunk` contains a {chunk}, if the command responses with nodes.

`messages` contains an array of additional messages the repository can include with its response.
Each message MUST have the following properties:

* `messageCode` is an <<{serialization}.adoc#id, id>>-compatible string identifying the message type.
 ##TODO: Do we limit the messageCode to pre-defined list or can a repo add its own?##
* `message` is a human-readable string describing the message.
* `data` is a flat object with arbitrary keys and values.
All values MUST be strings, the keys MUST be <<{serialization}.adoc#id, id>>-compatible.
A _messageCode_ might imply presence of specific keys in `data`.

Refer to <<bulk-json-schema>> for details.

== Message codes for all SerializationChunks
...
The repository MUST NOT validate the "meta-semantics" of the sent nodes, e.g. whether a Concept instance without parent is a partition, a feature instance can be part of its node instance, or a node mentioned as annotation is an Annotation instance.{fn-org226}

The repository SHOULD support changing <<{serialization}.adoc#meta-pointer, meta-pointers>>, e.g.  a node's `classifier.language`, a property's `property.version` or an enumeration literal's `key`.{fn-org69} #TODO Correct? Update issue#
...
===== Meta-pointer change not supported
.Success
false
...
.Data
[horizontal]
`old-language`:: `language` of existing meta-pointer
`new-language`:: `language` of attempted changed meta-pointer
`old-version`:: `version` of existing meta-pointer
`new-version`:: `version` of attempted changed meta-pointer
`old-key`:: `key` of existing meta-pointer
`new-key`:: `key` of attempted changed meta-pointer
...
[[bulk-json-schema]]
== JSON Schema for Bulk API responses
[source,json]
----
include::bulkapi.schema.json[]
----

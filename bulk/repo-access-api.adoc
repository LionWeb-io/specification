...
Optional parameter, defaults to `direct`.
[[languages.mode.direct]]
`direct`::: Only include the elements of `languages` themselves.
[[languages.mode.closure]]
`closure`::: Include the transitive closure of all elements of `languages`, i.e. the languages themselves and all (directly or indirectly) <<{m3}.adoc#Language.dependsOn, dependsOn>> languages.

.Result
<<{serialization}.adoc#SerializationChunk, SerializationChunk>> containing all languages according to `languages` and `mode` parameters as instances of <<{m3}.adoc#Language, M3 Language>>.
...
.Example request: Get details on two languages
[source, httprequest]
----
GET /bulk/languages?mode=direct
...
[source, httprequest]
----
GET /bulk/languages?mode=closure
...
The ids MUST exclude the <<{m3}.adoc#builtin-ids, built-in ids>>.

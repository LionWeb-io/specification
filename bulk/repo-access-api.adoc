...
#TODO: Add reference to model validation#
...
Depending on the command, it might contain a `chunk` or `nodeIds` property.
...
`chunk` contains a {chunk}, if the command responses with nodes (i.e. <<listPartitions>>, <<retrieve>>).

`nodeIds` contains an array of <<{serialization}.adoc#id, node ids>>, if the command responses with node ids (i.e. <<ids>>).

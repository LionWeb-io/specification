...
A server MAY consider the deleted node's id to be _unused_, and thus allow to re-use it.
A server also MAY disallow re-using previously deleted node ids.

[[client-ids]]
== Client ids
Each <<{arch}.adoc#client, client>> MUST  provide a unique _client id_ when connecting to the server.{fn-org241}
...
The server does not apply any uniqueness checks.
...
A client or server MAY reply with other, additional message kinds.
...
The server MUST NOT validate the metamodel constraints of the sent nodes.{fn-org226}

The server MUST support changing <<{serialization}.adoc#meta-pointer, meta-pointers>>, e.g.  a node's `classifier.language`, a property's `property.version` or an enumeration literal's `key`.{fn-org69}
...
Rationale: Otherwise, the server must track sessions, and run housekeeping on leases.
...
The server MUST return between one (inclusive) and `count` (inclusive) ids.

...
The bulk API *version 2026.1* is used to store and retrieve nodes in batches at the moment of invocation.{fn-org25}
...
For now, we don't include any specific protections against denial-of-service attacks.{fn-org237}
...
We call this part of the response <<successType, success flag>>.

Besides the main response, the repository can reply each command with zero or more <<additionalInfosType, additional infos>>.
...
[[depthLimitType]]depthLimit:: <<integerType>>? Limit the depth of retrieved subtrees.
...
[[commandIdType]]commandId:: Each _modifying_ command MUST contain a _[[commandId, command id]]command id_.{fn-org313}
...
[[successType]]success:: A boolean flag signaling _success_ or _failure_ with each response.
...
Lists all partitions{fn-org450} accessible in the repository.
...
.Request parameters
[horizontal]
[[listPartitions.depthLimit, `depthLimit`]]depthLimit:: <<depthLimitType>>? Optionally limit the depth of retrieved partitions.{fn-org425}
...
[[retrieve.depthLimit, `depthLimit`]]depthLimit:: <<depthLimitType>>? Optionally limit the depth of retrieved subtrees.

...
Instead of just forgetting about them, a server can optionally move them into a special place called _trash_.{fn-org217}
Orphans can be _recovered_ from there by re-inserting them into the original model tree.

== Trash lifetime
The server MAY delete nodes in _trash_ according to its own _garbage collection_ rules (e.g. after some time, or on memory pressure).
...
The server MAY choose to directly delete an orphan on some criteria, e.g. low resources.

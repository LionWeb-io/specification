...
7. Repository continues its response to query with id `qry-1` with another <<qry-ContinuedQueryResponse>> with chunk of 1 node: `nodeB` (details omitted).
This is the first in the sequence of continued chunks (`continuedChunkSequenceNumber: 0`) and there will be another continued chunk (`continuedChunkCompleted: false`).
8. Client receives the next split query result and buffers partial response.
9. Repository continues its response to query with id `qry-1` with another <<qry-ContinuedQueryResponse>> with chunk of 2 nodes: `nodeC`, `annotationD` (details omitted).
This is the second in the sequence of continued chunks (`continuedChunkSequenceNumber: 1`) and there will _not_ be another continued chunk (`continuedChunkCompleted: true`).

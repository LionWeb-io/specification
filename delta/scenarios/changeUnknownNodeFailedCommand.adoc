...
4. Server checks whether node id `node with spaces` is valid.
5. Client registers processed command id `cmd-1`.
6. Client informs editor of command submission.
7. Server rejects invalid node id.
8. Server sends <<evnt-Error>> event only to this participation with `invalidNodeId` error code, `invalid node id 'node with spaces'` message and sequence number `1`.

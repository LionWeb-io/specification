...
[[err-notAProperTree]]
==== Not a proper tree
The `DeltaJsonChunk` given by the client does not form a proper tree
...
[[err-childNotFound]]
==== Child not found
The `DeltaJsonChunk` given by the client does not cintain anode with the correct `parent`

.Technical name
`childNotFound`

[[err-unknownContainment]]
==== The containment cannot be found
The `parent` has no such containment

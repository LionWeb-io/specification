...
A client is a program connecting to a server.
...
We can use any (e.g. stateless, stateful) protocol, as long as it fulfills request/response characteristics for client-initiated query, command, and participation messages, and server-initiated broadcast characteristics for event messages.
...
The server assigns the participation id upon starting a new participation.
...
The server treats that command the same as from a client that was subscribed to the containing partition, i.e. applies it in accordance with the server's policy.
The server also sends out the same events to subscribed clients, thus _not_ to the sending client.{fn-org352}
...
A server MAY reply with other, additional message kinds.

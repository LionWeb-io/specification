...
* [[additionalInfo.reach]]`reach` describes who should receive this additional info.{fn-org421}
Its value MUST be one of:
+
[horizontal]
`clientToRepositoryOnly`:: A client sends this protocol message to the repository.
The repository MUST NOT deliver this protocol message to any client.
The repository MUST NOT use this value for self-initiated protocol messages.

`clientToAllClients`:: A client sends this protocol message to the repository.
The client MUST NOT use this value in <<queries>>, i.e. only in <<commands>>.
The repository MUST deliver this protocol message to all clients in the event corresponding to the client's command.
If the event is an <<evnt-ErrorEvent>>, the repository MUST deliver this protocol message to all recipients of the error event.
The repository MUST NOT use this value for self-initiated protocol messages.

`repositoryToSingleClient`:: A repository sends this protocol message to only one client.
Clients MUST NOT use this value.

`repositoryToAllClients`:: A repository sends this protocol message to all clients.
The repository MUST NOT use this value in <<queries>>, i.e. only in <<events>>.
Clients MUST NOT use this value.
...
This additional info is symmetric to <<{bulk}.adoc#Response.additionalInfos, additional infos in bulk API>>.

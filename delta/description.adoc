...
The continued chunks SHOULD be sent across the <<channel>> in increasing order of consecutive sequence number.
...
.Errors
* Split message already completed: continued chunk received with a higher consecutive sequence number than the first one for which the <<continuedChunkCompletedFlagType>> was `true`.
* Continued chunk sequence declared complete more than once: multiple continued chunks (with different consecutive sequence numbers) had their <<continuedChunkCompletedFlagType>> set to `true`.
* Received continued chunk with consecutive sequence number that was already seen.
* Continued chunks refers to a non-existent query/command/event.
...
* [[additionalInfo.data]]`data` is a map – meaning, a dictionary-like JSON object – with 0 or more arbitrary keys and values.

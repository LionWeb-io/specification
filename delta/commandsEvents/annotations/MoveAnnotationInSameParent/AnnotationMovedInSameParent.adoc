...
<<existing-nodes, Existing node>> <<AnnotationMovedInSameParent.movedAnnotation>> (previously inside <<AnnotationMovedInSameParent.parent>>'s annotations at <<AnnotationMovedInSameParent.oldIndex>>) has been moved inside <<AnnotationMovedInSameParent.parent>>'s annotations at <<AnnotationMovedInSameParent.newIndex>>.

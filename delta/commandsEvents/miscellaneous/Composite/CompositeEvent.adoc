...
We CAN nest composite events arbitrarily{fn-org360}, and CAN include <<evnt-ErrorEvent>> and/or <<evnt-NoOpEvent>>{fn-org491}.
...
* Any of the events contained directly in <<CompositeEvent.parts>> as well as any event that’s contained indirectly through <<evnt-CompositeEvent>> (or a <<evnt-custom>>) MUST NOT be a <<evnt-ContinuedEvent>>.
Subsequently, any (directly or indirectly) contained <<splittableMessage, splittable event>> MUST NOT have their <<splitFlagType>> set to `true`.{fn-org462}
...
(The <<evnt-NoOpEvent, `NoOp`>> and <<evnt-ErrorEvent, `Error`>> don't have command counterparts.)
]
...
[[CompositeEvent.sequenceNumber]]sequenceNumber:: <<eventSequenceType>>

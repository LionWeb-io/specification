...
* Any of the commands contained directly in <<CompositeCommand.parts>> as well as any command that’s contained indirectly through <<cmd-CompositeCommand>> (or a <<cmd-custom>>) MUST NOT be a <<cmd-ContinuedCommand>>.
Subsequently, any (directly or indirectly) contained <<splittableMessage, splittable command>> MUST NOT have their <<splitFlagType>> set to `true`.{fn-org462}
...
The commands this composite combines.
...
* <<CompositeCommand.parts>> contains <<continuedChunk, continued chunks>> or declares a <<splittableMessage>> as split{fn-org462}

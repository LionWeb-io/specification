...
* None of the <<CompositeEvent.parts>> is a <<evnt-CompositeEvent>>, also not through nesting through a <<evnt-CompositeEvent>> (or <<evnt-custom>>).
Subsequently, no contained <<splittableMessage>> (also not through nesting) have their <<splitFlagType>> set to `true`.{fn-org462}

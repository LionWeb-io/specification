...
* None of the <<CompositeCommand.parts>> may be a <<cmd-ContinuedCommand>>, also not through nesting through a <<cmd-CompositeCommand>> (or <<cmd-custom>>).
Subsequently, no contained <<splittableMessage>> (also not through nesting) should have their <<splitFlagType>> set to `true`.{fn-org462}

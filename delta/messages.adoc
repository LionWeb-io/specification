...
=== Parameter types
[horizontal]
[[protocolMessageType]]protocolMessage:: Optional <<protocolMessage>>.
...
Same format as <<{m3}.adoc#identifiers, node ids>>; must be unique per <<participation>>.
...
Same format as <<{m3}.adoc#identifiers, node ids>>; must be unique for all concurrent participations.

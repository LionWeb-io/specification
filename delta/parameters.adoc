...
[[nodeIdType]]NodeId:: A node represented by its <<{m3}.adoc#node-id, id>>.
...
[[resolveInfoType]]ResolveInfo:: <<{serialization}.adoc#Reference.reference.resolveInfo, ResolveInfo>> of a reference as string.
Can be `null`.

[[indexType]]Index:: Position inside a (list of) multiple link or annotations as integer >= 0.
Cannot be `null`.
Use value `0` for <<{m3}.adoc#multiplicity, single links>>.

[[indexOffsetType]]IndexOffset:: <<indexOffset, Relative position>> inside a (list of) multiple link or annotations as integer.
Cannot be `0` or `null`.
...
MUST contain a single [[anchorNode, anchor node]]_anchor node_, and all other nodes MUST be descendants of the _anchor node_.
...
[[continuedChunkCompletedFlagType]]ContinuedChunk{zwsp}CompletedFlag:: boolean flag whether this <<continuedChunk>> completes its <<splittableMessage>>.{fn-org425}

[[continuedChunkSequenceNumberType]]ContinuedChunk{zwsp}SequenceNumber:: orders connected <<continuedChunk, continued chunks>>.
...
[[commandSourceType]]CommandSource:: _(<<participationIdType>>, <<commandIdType>>)_ tuple{fn-org306} an event is based on.
This allows a client to know when its command has been processed (either successful or not).

[[eventSequenceType]]Event{zwsp}SequenceNumber:: <<event-sequence-number>> of this event.

[[freeIdType]]FreeId:: Free <<{m3}.adoc#node-id, node id>>.

[[deltaProtocolVersionType]]DeltaProtocol{zwsp}Version:: Version of the delta protocol, e.g. `2026.1`.

[[errorCodeType]]ErrorCode:: Code of an error.
Same format as <<{m3}.adoc#identifiers, identifiers>>; must be unique for reported errors.

[[errorMessageType]]ErrorMessage:: Human-readable message of an error.
...
[[countType]]count:: An integer value in the range of 0 to 2147483647 (i.e. 2^31^ − 1).
...
* Indirectly any command / event nested within <<cmd-CompositeCommand>> / <<evnt-CompositeEvent>>.{fn-org420}

...
* If we send <<removeChild>>, do we also need to send that for all descendants?
...
* Do we include the optional parameters?
...
* oldIndex: <<indexType>>
|
* `newIndex` is beyond (the number of `newContainment` entries in `parent`) - 1
* `movedChild` does not exist
* `parent` does not exist
* `movedChild` is not a child of `parent` in `containment`
* `oldIndex` is beyond (the number of `newContainment` entries in `parent`) - 1
